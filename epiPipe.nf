...
def helpMessage() {
    ...
    Input (minimum one of these, can be mixed):
        ...
        --genewizMap [str]           CSV file descrbibing the LIMS id to S3 path on GeneWiz bucket. Fields (no header): LIMS_ID,LIBRAY_ID,S3_path_R1,S3_path_R2
    ...
    Alignment:
        --genome [str]               Name of the genome to use. Possible choice: hg38, hg19, mm10, dm3, hg38_hla, susScr11, rn6, GRCg6a. Default: hg38.
...
if (params.genome =~ /hg19|hg38|mm10|dm3|hg38_hla|susScr11|rn6|GRCg6a/){
    Channel
	.fromFilePairs("${HOME}/ebs/genome/nextflow/bwa-mem2-index/${params.genome}/*.{amb,sa,pac,ann,bwt,fa}", size: -1, checkIfExists: true)
	.ifEmpty { exit 1, "BWA index not found: ${params.genome}" }
	.set { bwa_index }
    Channel
	.fromPath("${HOME}/ebs/genome/nextflow/bwa-mem2-index/${params.genome}/${params.genome}.fa", checkIfExists: true)
	...
	input:
	path(genome) from Channel
	    .fromPath(params.genome, checkIfExists: true)
	    .ifEmpty {     exit 1, "Only hg38, hg19, hg38_hla,  mm10, susScr11,  and dm3 genomes are currently offered" }
	
	output:
	tuple val(id), path("*") into bwa_index
	
	script:
	id="genome_idx"
	"""
	bwa-mem2 index -p ${id} ${genome}
	"""
    }
