...
process make_mapFiles {
    ...
    output:
    tuple id, val(res), path("*.rmap"), path("*.baitmap") into mapFiles_ch
...
process make_design {
    ...
    input:
    tuple id, val(res), path(rmap), path(baitmap) from mapFiles_ch

    output:
    tuple id, val(res), path(rmap), path(baitmap), path("${res}kDesingFiles*") into design_ch
...
process run_Chicago {
    ...
    input:
    tuple id, path(bam), val(id2), val(res), path(rmap), path (baitmap), path(designFiles) from cleanBam2_ch
	.combine(design_ch)
